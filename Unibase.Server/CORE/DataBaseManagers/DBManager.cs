...
namespace UniBase.CORE.DataBaseManagers
{
    public class DBManager
    {
        ...
        public async Task<List<JournalHeaderDB>> GetJournalHeaderData(int FaculityID, int semestr,  string startDate = "27.12.2005",  string EndDate = "27.12.2024", int pageNum = 0, string name = "")
        {
            var first = 0;
            var second = 40;
            string nameFilter = " ";
            ...
            if (name != "")
            {
                nameFilter = $"AND Prepods.ФИО LIKE '%{name}%'";
            }
            if(semestr != null)
            {
                semestFilter =  $" AND PrepJournal.Семестр = {semestr}";
            }
            var query = _context.JournalHeader.FromSqlRaw($@"
                                                            ...
                                                            SELECT * FROM (
                                                                ...
                                                                WHERE
                                                                     Groups.Код_Факультета = {FaculityID}
                                                                    {semestFilter}
        ...
        public int getJournalEvalCount(int journalId, string lectionType)
        {
        ...
        public async Task<int> GetNagrHours(int NagrId,string lectionType)
        {
        ...
        public async Task<int> GetStudentCount(int NagrId, string lectionType)
        {
                                            ...
                                            int semestr = 1,string name = "")
        {
            ...
            string query = $@"
                        ...
                        FROM [Деканат].[dbo].[ЖурналПреподавателя] PrepJournal
                                                             ...
                                                             Groups.Код_Факультета = {faculity}
                                                                    {semestFilter}
                                                                    AND PrepJournal.Код IN (
                                                                        SELECT КодЖурнала FROM [Деканат].dbo.ЖурналДаты
                                                                        WHERE Дата BETWEEN @a AND @b
