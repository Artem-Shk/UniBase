...
var page_num = 1
// какой то костыль не красиво TODO:Сделайть лучше
function Body() {
    const [journals, setJournals] = useState([]);
    const [currentPage, setCurrentPage] = useState(1)
    const RefFilterObj = useRef({
        "findline": "",
        "datepicker": ['01.01.2005', new Date().toLocaleDateString("ru-RU")],
        "semestr": 2,
    })
    const [rowsCount, setRowsCount] = useState(undefined);
    useEffect(() => {
        const fetchData = async () => {
            await UpdateJournals(28);
            const count = await RowsCount();
            setRowsCount(count);
        };
        fetchData();
    }, []);
    ...
    async function UpdateJournalsWithFilter(kaf_id) {
        ...
        const response = await fetch('https://localhost:5000/api/JournalData/GetJornalsHeaders' + '/' + kaf_id
            + '/' + page_num
 
            + '/' + RefFilterObj.current.datepicker[0]
            + '&' + RefFilterObj.current.datepicker[1]
            + '/' + RefFilterObj.current.semestr
            + '/' + RefFilterObj.current.findline);
        const data = await response.json(); 
        
        if (data != undefined) {
 
            setJournals(data);
            setRowsCount(await RowsCount())
    ...
    async function RowsCount() {
        const querySt = 'https://localhost:5000/api/JournalData/GetRowCount/'
            + 28
            + '&' + RefFilterObj.current.datepicker[0]
            + '&' + RefFilterObj.current.datepicker[1]
            + '/' + RefFilterObj.current.semestr
...
function PartOfList({ prepodName, GroupName, usercount, disciplineName, attendance, stat, journal_id, nagr_idList, lectionTypes }) {
    ...
    async function UpdateBody(item, journal_id, nagr_id) {
        const today = new Date().toLocaleDateString("de-DE")
        var response = await fetch('https://localhost:5000/api/JournalData/GetJornalBody/' + journal_id + '&' + today + '&' + nagr_id + '&' + item);
        const data = await response.json();
        console.log(data)
        setData(data);
    }
    function SuperAnaliticCard({ journal_id, nagr_idList, lectionTypes, data }) {
            ...
            <div className={styles.super_analictic}>
                <div className={styles.super_analictic_calendar}>
                    <div className={styles.super_analictic_calendar_Button}>
                        {lectionTypes.map((item, index) => <Button
                            onClick={
                                () => UpdateBody(item, journal_id, nagr_idList[index])
                            }
                            text={item}
                        >
                        </Button>)}
    ...
    async function UpdateJournals(journal_id, nagr_id) {
        ...
        if (AnaliticCardVisible === true) {
            response = await fetch('https://localhost:5000/api/JournalData/GetJornalBody/' + journal_id + '&' + today + '&' + nagr_id + '&' + lectionTypes[0]);
            const data = await response.json();
            setData(data);
        }
    } 
    ...
    return (
        <div style={{ display: "flex", width: '100%', flexDirection: 'column' }}>
            ...
            {!AnaliticCardVisible && (
                ...
                <SuperAnaliticCard journal_id={journal_id}
